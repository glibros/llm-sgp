...
def selBestIndeces(individuals, k, fit_attr="fitness"):
    """Select the *k* best individuals among the input *individuals*. The
    list returned contains references to the input *individuals* and their indices.

    :param individuals: A list of individuals to select from.
    :param k: The number of individuals to select.
    :param fit_attr: The attribute of individuals to use as selection criterion
    :returns: A list containing the k best individuals and a list of their indices
    """
    # Enumerate the individuals and sort them by their fitness attribute in descending order
    sorted_individuals_with_indices = sorted(enumerate(individuals), key=lambda x: getattr(x[1], fit_attr), reverse=True)
    # Slice the list to keep only the top k individuals
    best_individuals_with_indices = sorted_individuals_with_indices[:k]
    
    # Separate the individuals and their indices
    best_individuals = [individual for index, individual in best_individuals_with_indices]
    indices = [index for index, individual in best_individuals_with_indices]
    
    return best_individuals, indices
